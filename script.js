...
$(window).resize(function () {
  $(".slider_inner").css(
  "left",
  $(document).width() / 2 - $(".slider_inner__slide").width() / 2);
...
$(".slider_inner").mousemove(function (e) {
  if (dragging) {
    ...
    $(
    ".slider_inner__slide:nth-of-type(" +
    parseInt(index + 1) +
    ") .image .overlay, .slider_inner__slide:nth-of-type(" +
    parseInt(index + 1) +
    ") .image .title, .slider_inner__slide:nth-of-type(" +
    parseInt(index + 1) +
    ") .image .cats, .slider_inner__slide:nth-of-type(" +
    parseInt(index + 1) +
    ") .image .button").
    css("opacity", 1 - Math.abs(difference / 200));
    ...
    $(".slider_inner").css(
    "transform",
    `translateX(${difference + endPosition}px) translateY(120px)`);
...
$(".slider_inner").mouseup(function () {
  ...
  if (difference < -160) {
    ...
    if (index < 7) {
      ...
      $(".slider_inner").css(
      "transform",
      `translateX(${threshold}px) translateY(120px)`);
  ...
  dragging = false;
  console.log(
  ".slider_inner__slide:nth-of-type(" + parseInt(index + 1) + ")");
...
function drawMouseSpeedDemo() {
  ...
  var mdraw = function () {
    ...
    if (lastmousetime && lastmousetime != timenow) {
      ...
      //console.log(pps)
      if (dragging) {
        let velocity = 0.5 - pps / 40000;

        $(".slider_inner__slide").css(
        "transform",
        "rotateY(" + direction + pps / 110 + "deg) scale(1)");
